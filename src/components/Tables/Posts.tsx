...
const TableTwo = () => {
  ...
  const fetchPosts = async () => {
    try {
        ...
        let response;
        if(role == "super_agent"){
           response = await axios.get<Post[]>('http://148.113.194.169:5000/api/posts/allsuper', config);
        }else {
          response = await axios.get<Post[]>('http://148.113.194.169:5000/api/posts/all', config);
        }
  ...
  const handleAddPost = async () => {
    try {
      ...
      toast.info('En attent de validation de l\'administrateur dans 24h');
  ...
  const handleChangeStatus = async (postId: string, newStatus: string) => {
    try {
      ...
      const response = await axios.put(
        `http://148.113.194.169:5000/api/posts/${postId}/status`,
        { status: newStatus },
        config
      );
  ...
  {role === 'super_agent' && (
    <button 
      onClick={() => handleChangeStatus(post._id, post.validate === 'approved' ? 'hide' : 'approved')} 
      className={`hover:text-primary ${post.validate === 'approved' ? 'text-green-600' : 'text-gray-600'}`}
    >
      {post.validate === 'approved' ? 'Approved' : 'Hide'}
    </button>
  )}
</div>
